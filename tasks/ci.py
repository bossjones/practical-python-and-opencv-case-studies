...
def pylint(ctx, loc="local", tests=False, everything=False, specific=""):
    """
    pylint practical-python-and-opencv-case-studies folder
    Usage: inv ci.pylint
    """
    env = get_compose_env(ctx, loc=loc)
    ...
    if tests:
        ...
        ctx.run(
            "pylint --output-format=colorized --rcfile ./lint-configs-python/python/pylintrc tests practical_python_and_opencv_case_studies"
        )
    elif specific:
        ctx.run(
            f"pylint --output-format=colorized --disable=all --enable={specific} --rcfile ./lint-configs-python/python/pylintrc tests practical_python_and_opencv_case_studies"
        )
    else:
        ctx.run(
            "pylint --output-format=colorized --disable=all --enable=F,E --rcfile ./lint-configs-python/python/pylintrc practical_python_and_opencv_case_studies"
        )
...
def mypy(ctx, loc="local", verbose=0):
    """
    mypy practical-python-and-opencv-case-studies folder
    Usage: inv ci.mypy
    """
    env = get_compose_env(ctx, loc=loc)
...
def black(ctx, loc="local", check=False, debug=False, verbose=0, tests=False):
    ...
    _cmd += "practical_python_and_opencv_case_studies"
...
def isort(ctx, loc="local", check=False, dry_run=False, verbose=0, diff=False):
    """
    isort practical-python-and-opencv-case-studies module. Some of the arguments were taken from the starlette contrib scripts. Tries to align w/ black to prevent having to reformat multiple times.
    ...
    _cmd += " practical_python_and_opencv_case_studies tests"
...
def pre_start(ctx, loc="local", check=True, debug=False):
    """
    pre_start practical-python-and-opencv-case-studies module
    """
    env = get_compose_env(ctx, loc=loc)
    ...
    # ctx.run("python practical_python_and_opencv_case_studies/api/tests_pre_start.py")
...
def pytest(
    ...
    _cmd += r" --cov-config=setup.cfg --verbose --cov-append --cov-report=term-missing --cov-report=xml:cov.xml --cov-report=html:htmlcov --cov-report=annotate:cov_annotate  --showlocals --tb=short --cov=practical_python_and_opencv_case_studies tests"
