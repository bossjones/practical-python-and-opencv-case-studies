...
import cv2
from faster_rcnn import config, data_generators, roi_helpers
...
def format_img(img, C):
    ...
    (resized_width, resized_height, ratio) = data_generators.get_new_img_size(
    ...
    for key in bboxes:
        ...
        if new_probs[jk] > 0.55:
            ...
            if export:
                ...
                cv2.putText(
                    img, textLabel, textOrg, cv2.FONT_HERSHEY_DUPLEX, 1, (0, 0, 0), 1
                )
...
if __name__ == "__main__":
    ...
    for idx, img_name in enumerate(sorted(os.listdir(options.test_path))):
        ...
        cv2.imwrite(
            "./results_test/result_{}.png".format(img_name.replace(".png", "")), img
        )
