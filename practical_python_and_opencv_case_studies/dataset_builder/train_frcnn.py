...
from faster_rcnn import config, data_generators, losses as losses, resnet as nn
...
with open(C.config_filename, "wb") as config_f:
    ...
    print(
        "Config has been written to {}, and can be loaded when testing to ensure correct results".format(
            C.config_filename
        )
    )
...
classifier = nn.classifier(
    shared_layers, roi_input, C.num_rois, nb_classes=len(classes_count), trainable=True
)
            ...
            try:
                ...
                if iter_num == epoch_length:
                    ...
                    if C.verbose:
                        ...
                        print(
                            "Classifier accuracy for bounding boxes from RPN: {}".format(
                                class_acc
                            )
                        )
                    ...
                    if curr_loss < best_loss:
                        if C.verbose:
                            print(
                                "Total loss decreased from {} to {}, saving weights".format(
                                    best_loss, curr_loss
                                )
                            )
                        best_loss = curr_loss
